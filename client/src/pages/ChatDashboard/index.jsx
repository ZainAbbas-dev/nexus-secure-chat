...
const getRoomKey = (roomId) =>
  CryptoJS.HmacSHA256(roomId, "nexus_prime_2026_secure_layer").toString();

const generateRoomId = (me, them) => {
  const id1 = String(
    me?.email || me?.username || me?.full_name || me?.id,
  ).toLowerCase();
  const id2 = String(
    them?.email || them?.username || them?.full_name || them?.id,
  ).toLowerCase();
  return [id1, id2].sort().join("_");
};

const isSpam = (text) =>
  /(?:verify.*account|update.*payment|password.*reset|crypto.*giveaway|click.*link.*win|http:\/\/\d+\.\d+\.\d+\.\d+|free.*money)/i.test(
    text || "",
  );
...
export default function ChatDashboard() {
  ...
  const {
    ...
    draw,
    clearCanvas,
  } = useWhiteboard(socket, room, room ? getRoomKey(room) : null);

  const scrollToBottom = () =>
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });

  const decryptMessage = (ciphertext, roomId) => {
    if (!ciphertext || !ciphertext.startsWith("U2Fsd") || !roomId)
      return ciphertext || "";
    try {
  ...
  useEffect(() => {
    ...
    socket.on("online_users", (emails) =>
      setOnlineUsersSet(new Set(emails.map((e) => e.toLowerCase()))),
    );
    socket.on("user_online", (email) =>
      setOnlineUsersSet((prev) => new Set(prev).add(email.toLowerCase())),
    );
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => setUsers(res.data.filter((u) => u.email !== user.email)))
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/api/messages/${roomId}`)
      .then((res) =>
        setMessages(
          res.data.map((msg) => ({
            ...msg,
            message: decryptMessage(msg.message, roomId),
          })),
        ),
      )
      .catch(() => {});
    axios
      .get(`${API_URL}/api/trust/${roomId}`)
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", (data) => {
      ...
      setMessages((prev) => [
        ...prev,
        { ...data, message: decryptMessage(data.message, correctRoomId) },
      ]);
    ...
    socket.on("message_delivered", ({ id, tempId }) =>
      setMessages((prev) =>
        prev.map((msg) =>
          (msg.id === id || msg.tempId === tempId) && msg.status !== "read"
    ...
    socket.on("message_read", ({ id, tempId }) =>
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === id || msg.tempId === tempId
            ? { ...msg, status: "read" }
            : msg,
        ),
      ),
    );
    socket.on("message_deleted_everyone", (id) =>
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === id || msg.tempId === id
    ...
    return () => {
      ...
      socket.off("receive_code_update");
  ...
  const sendMessage = async (e) => {
    e.preventDefault();
    if (currentMessage.trim() !== "" && room && socket) {
      await emitMessage(currentMessage);
      setCurrentMessage("");
      socket.emit("stop_typing", room);
    }
  };
  ...
  const handleDeleteMessage = (type) => {
    ...
    if (type === "me")
      setMessages((prev) =>
        prev.map((m) =>
          m.id === msgId || m.tempId === msgId
            ? { ...m, deleted_for: [...(m.deleted_for || []), user.full_name] }
            : m,
        ),
      );
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === msgId || m.tempId === msgId
            ? { ...m, is_deleted: true, message: "[DELETED]" }
            : m,
        ),
      );
  ...
  const handleClearChat = () => {
    socket.emit("clear_chat", { room, requester: user.full_name });
    setMessages([]);
  ...
  return (
    <div className="flex h-screen bg-dark-900 text-white overflow-hidden relative">
      ...
      <div
        className={`flex-1 flex flex-col bg-dark-900/95 transition-all z-10 ${!activeChat ? "max-sm:hidden" : ""}`}
      >
        {activeChat ? (
          <>
            <ChatHeader
            ...
            <MessageInput
              ...
              insertCodeTemplate={() => {
                setCurrentMessage("LIVE_CODE_BLOCK");
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-500">
            ...
            <h2 className="text-xl font-semibold">Nexus Secure</h2>
          </div>
        )}
      </div>
      ...
      {showCodeEditor && (
        <LiveCodeEditor
          ...
          handleCodeChange={(val) => {
            setLiveCode(val);
            socket.emit("code_update", { room, code: val });
          }}
        />
      )}
      {showWhiteboard && (
      ...
      {showClearChatModal && (
        <div className="absolute inset-0 bg-dark-900/80 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-dark-800 p-6 rounded-3xl border border-gray-700 text-center flex flex-col items-center w-80 shadow-2xl">
            <h3 className="text-lg font-bold text-white mb-2">
              Clear this chat?
            </h3>
            <p className="text-sm text-gray-400 mb-6">
              Messages will only be removed from your device.
            </p>
            <div className="flex flex-col gap-3 w-full">
              ...
              <button
                ...
                className="w-full bg-dark-700 hover:bg-dark-600 text-white py-3 rounded-xl font-medium transition"
              >
                Cancel
              </button>
            </div>
      ...
      {showDeleteModal && (
        <div className="absolute inset-0 bg-dark-900/70 z-50 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-dark-800 p-6 rounded-3xl border border-gray-700 text-center flex flex-col items-center w-80 shadow-2xl">
            ...
            <div className="flex flex-col gap-3 w-full">
              {showDeleteModal.author === user?.full_name && (
                <button
                  onClick={() => handleDeleteMessage("everyone")}
                  className="w-full bg-transparent border border-red-500 text-red-500 hover:bg-red-500 hover:text-white py-3 rounded-xl font-medium transition"
                >
                  Delete for everyone
                </button>
              )}
              ...
              <button
                ...
                className="w-full bg-transparent text-gray-400 hover:text-white py-2 mt-2 transition"
              >
                Cancel
              </button>
            </div>
      ...
      <CallOverlay
        ...
        endCallLocal={endCallLocal}
        acceptCall={acceptCall}
        localStream={localStream}
        remoteStream={remoteStream}
        endCallNetwork={endCallNetwork}
      />
    </div>
  );
}
